...
import { createBatchClasses, deleteClassFromDirectory, getAllWorkers, getAllDepartmentsList, saveClassToDirectory } from '../../db/indexedDB';
            ...
            >
              <div className="col-span-3">
                ...
                <select
                  ...
                  className="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-900 outline-none focus:border-indigo-600 cursor-pointer"
                >
            ...
            <form onSubmit={handleSaveEditClass} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  Department (Authorized List)
                </label>
                <select
                  value={editClassDept}
                  onChange={(e) => setEditClassDept(e.target.value)}
                  className="w-full px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs font-bold text-slate-900 outline-none focus:border-indigo-600 cursor-pointer"
                >
                  {approvedDepartments.map(d => (
                    <option key={d} value={d}>{d}</option>
                  ))}
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  Departments are controlled authoritatively by the General Secretary & General Superintendent.
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-900 outline-none focus:border-indigo-600"
                  required
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  Update Class Password (Optional)
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-3.5 py-2 bg-white border border-slate-300 rounded-xl text-xs font-medium text-slate-900 outline-none focus:border-indigo-600 font-mono"
                />
                <p className="text-[10px] text-slate-400 mt-1">
                  Enter 6+ characters only if resetting the class credential for teachers/secretaries.
                </p>
              </div>
              ...
              <div className="flex items-center justify-end gap-2.5 pt-2">
                ...
                <button
                  type="submit"
                  disabled={isSavingEdit}
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs transition flex items-center gap-1.5 disabled:opacity-50"
                >
                  {isSavingEdit ? (
                    <>
